...
그때 알게된 Alamofire 라는 훌륭한 라이브러리를 알게 되었다.
...
![]({{ site.baseurl }}/assets/img/post/2018-07-21/img1.png)
> 이해가 잘안된다면, "내가 주소창에 URL을 입력(요청)했더니 위와 같은 화면(응답)이 나왔다" 정도로?? 생각하자. (이 부분은 설명이 부족한 것이니, 아래 내용이 이해가 안될경우에만 이 정도로만 생각하고, 아래에서 추가 설명함)
...
그래서 위에서 switch문을 썼을때 2가지만 써도 `default일 경우 처리하세요`라는 경고를 안띄운다. <br>
isSuccess는 성공 여부를 <br>
isFailur는 실패 여부를 <br>
value는 받아온 값이며 error는 Error타입이다, 이 둘중 한가지는  nil이기 때문에 옵셔널`?`으로 감싸져있다.
...
다운캐스팅 할때 `as?`을 써줌으로써 실패시 nil로 바뀌게 될것이고 따라서, 각 if문의 조건은 nil로 떨어짐으로 이 경우엔 swift에선 해당 라인을 실행 안한다고한다! 훌륭해.
...
새로 배운 기술이다. 브레이크 포인트에 lldb(Low Lebel Debugger) 명령어를 입력해주면, 해당 부분 가서 알아서 해당 명령어를 실행해준다. 해당 명령어 입력부분 아래에 옵션 체크를 해주면 원래는 break 걸면 해당 라인에서 멈춰야하지만 명령어 실행 후 자동으로 다시 진행한다. 
...
위에서 했던 방식대로 진행하면 이렇다. 하는건 쉽다. 하는 것만 쉽다.
...
# Codable
Codable은 Swift4에서 추가된것이다. 이것 사용하기전엔 위에처럼 비효율적이지만 어쩔수없이 사용을 했을것이다!(추측), 이것을 알게된건 let's Swift라는 컨퍼런스? 에서였다. 카카오톡, 라인, 토스 등등 개발자들이 짧게 20~30분씩 강연들을 했었던 것이었고, 난 학생이었기 때문에 반값? 수준으로 갔었던것 같다. 이때 옆자리분이 여기 슬랙따로 있나요? 했을때, 당시 나는 슬랙을 한번도 안써봐서 못알아 들었던 기억이 난다.
...
Swift는 읽기도 참 좋다!
>JSONDecoder()를 사용할거고, 데이터(from 인자)로부터 정한 타입(type 인자)으로 디코딩한다
